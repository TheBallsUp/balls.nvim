...
--- @field load_on fun(self: balls.Plugin, events: string | string[], opts?: balls.EventOpts) Sets up an autocmd to load this plugin on specific events.
...
command("BallsList", function()
	require("balls").list()
end, "Opens a floating window listing all registered plugins.")

command("BallsInstall", function()
	require("balls").install()
end, "Installs any registered plugins that aren't installed yet.")

command("BallsUpdate", function()
	require("balls").update()
end, "Updates any registered and installed plugins.")

command("BallsSync", function()
	require("balls").sync()
end, "Syncs registered plugins with what's actually installed locally.")

command("BallsClean", function()
	require("balls").clean()
end, "Cleans up any still installed but not registered plugins.")
